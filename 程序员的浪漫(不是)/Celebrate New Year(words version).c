...
int main() {
  Sleep(3000);  // Free up time to record a video, remove this line is OK,
                // without error happens.
  ...
  char s_19[] = {
      "\t\t\t\t\t\t\t                                                          "
      "       2020 New Year"};
  char *s_set[] = {s_1,  s_2,  s_3,  s_4,  s_5,  s_6,  s_7,  s_8,  s_9, s_10,
                   s_11, s_12, s_13, s_14, s_15, s_16, s_17, s_18, s_19};

  char s[9] =
      "color ";  // define a string to replace command "system("color xx")",so
                 // can easy change colors It needs at least 9 units to store the
                 // string, cause \0 need to store, too.
  ...
  for (int i = 0; i < N; i++)  // make a flicker effect,shine N times
  {
    ...
    puts("\t\t\t\t\t\t                                   ★ 2020春节快乐! ★");
    // Caution: Use printf() to replace puts(), the text will only be printed
    // once
    for (int i = 0; i < 81474400; i++)
      ;  // make the flicker effect softer and slower
  }

  puts("\n\n\t\t\t\t\t\t\t亲爱的计安的小伙伴们:\n");  // Use printf() to replace
                                                      // puts() will only print
                                                      // the line once TO_DO: I
                                                      // don't know why now, may
                                                      // be related to the
                                                      // printf() function
  puts("\t\t\t\t\t\t\t\t新春快乐!\n");
  s[6] = '4';  // change background color when display the main words, 4--red
               // Don't put this statement in the for(; ;) loop, it will meet
               // some errors
